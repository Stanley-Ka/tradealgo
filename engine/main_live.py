...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        ...
        type=str,
        default="",
        help="Features parquet path for latest signals",
    )
    p.add_argument(
        "--model-pkl", type=str, default="", help="Trained meta model pickle"
    )
    p.add_argument("--oof", type=str, default="", help="OOF parquet (for calibrators)")
    p.add_argument(
        "--news-sentiment", type=str, default="", help="Optional sentiment CSV/Parquet"
    )
    p.add_argument(
        "--top-k", type=int, default=20, help="Number of names to print as signals"
    )
    return p.parse_args()
